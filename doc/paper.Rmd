...
pandoc.table(
  ...
  full_width = FALSE,
  caption = "(\\#tab:example-table) An table must include the table tag in order to cross-reference it correctly."
)
```
...
```{r}
cap <- "A caption for an example figure"
```

```{r example-figure, fig.cap=cap}
plot(cars)
```
